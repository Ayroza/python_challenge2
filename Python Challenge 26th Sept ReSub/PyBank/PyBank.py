...
file_to_save = os.path.join('Analysis', 'pybank_analysis.txt')
with open(file_to_save, 'w') as txtfile:
    txtfile.write('Financial Analysis\n')
    txtfile.write('----------------------------\n')
    txtfile.write(f"Total Months: {len(total_number_months)}\n")
    txtfile.write(f"Total: ${total_profit_amount}\n")
    txtfile.write(f"Average Change: {sum(profit_loss_changes) / len(profit_loss_changes)}\n")
